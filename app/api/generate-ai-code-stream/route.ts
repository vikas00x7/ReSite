...
const ollama = (createOpenAI as any)({
...
5. Make the requested change immediately`;
        ...
        // Build messages for model providers
...
If you're running out of space, generate FEWER files but make them COMPLETE.
 It's better to have 3 complete files than 10 incomplete files.`
        ...
        // If using Ollama, call the OpenAI-compatible chat completions endpoint directly
        let ollamaTextAsyncIterable: AsyncIterable<string> | null = null;
        if (isOllama) {
          const ollamaUrl = (process.env.OLLAMA_BASE_URL || 'http://localhost:11434') + '/v1/chat/completions';
          ...
          ollamaTextAsyncIterable = sseToText();
        }
        
        const result = isOllama ? null : await streamText(streamOptions);
        ...
        const textStream = isOllama ? (ollamaTextAsyncIterable as AsyncIterable<string>) : result!.textStream;
...
Provide the complete file content without any truncation. Include all necessary imports, complete all functions, and close all tags properly.`;
                ...
                const completionModelId =
                  completionClient === openai
                    ? (model.startsWith('openai/') ? model.replace('openai/', '') : 'gpt-5')
                    : completionClient === anthropic
